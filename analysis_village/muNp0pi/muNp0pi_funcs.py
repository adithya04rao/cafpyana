...
def slcfv_cut(df, det):
    if "ICARUS" in det and "Run4" in det:
        run = 4
    elif "ICARUS" in det and "Run2" in det:
        run = 2
    elif "SBND" in det:
        run = 1
    else:
...
def particle_classification(df, cut_m, cut_p, muon_min_len_cut, muon_max_len_cut):
    ...
    muons = ((df[('pfp', 'trk', 'chi2pid', 'I2', 'chi2_muon')] < cut_m) & (df[('pfp', 'trk', 'chi2pid', 'I2', 'chi2_proton')] > cut_p) & (df[('pfp', 'trk', 'len')] > muon_min_len_cut) & (df[('pfp', 'trk', 'len')] < muon_max_len_cut))
...
def get_nuE_reco(df):
    ...
    def neutrino_energy(mu_p, mu_dir_x, mu_dir_y, mu_dir_z, p_p, p_E, p_dir_x, p_dir_y, p_dir_z):
        ...
        return mu_E + (df.slc.reco.np.groupby(level=[0,1,2]).first() * (p_E - PROTON_MASS + BE)) + ET
...
def muNp0pi_pipeline(df, muon_pid_cut, proton_pid_cut, muon_min_len_cut, muon_max_len_cut, det):

############## Some correction, ignore ################
#    truth_cols = [
#        ('slc', 'truth', 'nn', '', '', ''),
#        ('slc', 'truth', 'np', '', '', ''),
#        ('slc', 'truth', 'nmu', '', '', ''),
