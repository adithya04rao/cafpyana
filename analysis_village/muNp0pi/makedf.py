...
from analysis_village.muNp0pi.muNp0pi_funcs_df import muNp0pi_pipeline
...
def make_muNp0pidf(f: pd.DataFrame, save_track_truth: bool=False) -> pd.DataFrame:
        ...
        | (
            ...
            & (pandora_df.columns.get_level_values(1).isin(
                ...
                (pandora_df.columns.get_level_values(2).isin(
                    ["pdg", "E", "baseline", "genie_mode", "q0_lab", "modq_lab", "Q2", "nproton"]
                )))
        )
    )
    )
    ...
    recodf = muNp0pi_pipeline(recodf, 30, 80, 40, 400, "SBND")
